...
**M2-T0 status (preconditions bundle, `.superpowers/sdd/2026-08-02-m15-identity-amendment/task-m2t0-report.md`).** **(1) CLEARED** — the runtime stamps the pinned axis on the bound container as `data-suitemate-v3-edit-grid-axis` (comma-joined), cleared by name in `removeEditGrid`. It ships **unconditional, not debug-flagged**: the ids are already public in-page in `{machine}fields`, so the stamp adds no exposure and a flag would only add a second thing to get wrong on a live probe. Contract 51 → 52. **(3) CLEARED** — `runtime.js:365` refuses below two columns ahead of the signature compare, returning `true` (today's behaviour on an empty axis); `applyAll(table, [])` is unreachable. **(8) PART-CLEARED** — the `readColumnIdsFrom` selector-injection guard at `:139` now has behavioural coverage; the round-trip's committed home and the duplicate-ids harness check are still owed. **(7) CLEARED** (escalated as an omission from the bundle, upheld, folded in as a second commit) — `removeEditGrid` resets `saveQueue` to a fresh resolved promise, and `enqueueSave` now hands the caller a promise that still carries the operation's own rejection while storing that promise with the rejection swallowed, so one failed write can neither poison the queue behind it nor escape as an unhandled rejection when a writer fires and forgets. **Read the guarantee narrowly:** the plumbing only ensures a continuation resuming after teardown finds a reset queue and cannot chain onto the old mount's; the in-flight operation is **not** cancelled and **not** generation-checked there. The operation-side `isCurrent`/generation guard is **M2's writer task's** to add. **(2), (4), (5), (6) unchanged.** Gates at the bundle: 264/264, 28 baselines 0.000 %, fixture round-trip 16/16 with 0 console diagnostics.

## Edit Mode Table Enhancements: Milestone M3 (hide/show columns)

Status: Code adversarially reviewed to MERGE; MECHANISM live-verified on the locked record (independent adjudication: every exercised gate PASS); checkpoint DECLARATION pending the adjudicator's R1–R5 coverage closures (below)

Date: 2026-08-04

Commits `a1c0c9c..bc8e370` on `feature/edit-mode-table-enhancements` (inclusive), shipped across two sessions with a deletion in between: the branch was deleted 2026-08-03 evening and recovered from the reflog at `4cf0a1c` the same night, per the resume path prescribed in `docs/EDIT-MODE-BRANCH-STATUS.md`. Captain on Fable 5; implementer, reviewer and live-adjudication subagents on Opus 5 throughout.

### Included

- Hide/show runtime surface (Task 16 shape): Columns menu (43 label rows keyed by decoded column id, ticks seeded from the STORED set), hidden-column chips, class-only hide (`suitemate-v3-edit-grid-col-hidden`, `display: none !important`), force-reveal predicate `forceRevealed() = isLineOpen() || isDirty()` with the dirty scan wired across EVERY data row (the entry row counts only when dirty, by design — the brief's focusin rule 1 is unimplementable on this form, widgets materialize per cell on click).
- `4cf0a1c` (committed unreviewed last session) was reviewed this session: the original Critical (ADJUDICATION #20 failing across a repaint) is CLOSED, mutation-proven — but the review found the fix opened a NEW Critical of the same laundering class: the layout guard read `tableLayout === "fixed"` alone, so a teardown landing mid-buildtable (no header row: core refuses at `core.js:871-873` BEFORE the restore clears the layout, while the state clears ran anyway) left a stale `fixed` that starved every later mount into a partial plan — hidden cell renders 0, takes the 50px floor, reveal+drag stores floor+delta. A REGRESSION vs the pre-guard tree.
- `bc8e370` closes it, reviewed to MERGE across three rounds: `ownsLayout` ownership conjunct on the guard; unconditional `tableLayout` clear in `removeEditGrid` OUTSIDE the shared try; `releaseBindings` and `clearAxisStamp` run off a stashed container out there too (the teardown sweep's exact-attribute selector cannot reach the axis stamp — `clearAxisStamp` is its only remover). Plus the reviewer's surface sweep: scope key LATCHED per mount (axis-pin precedent — a session-script hiccup can no longer relabel the layout, reseed the hidden set empty, or aim writes at a shadow key); `forcedRows` (zero call sites) deleted and its guarantee wired into the live path; `applyHidden`'s boolean recorded (`hideIncomplete`) and consulted by BOTH signature early-returns (install and focusin); Columns menu closes on outside click and Escape (document listeners on the resize-listener precedent, released in `closeColumnMenu` and by name at teardown); `fieldIsDirty`'s untested quadrant (un-ticking a default-checked box) pinned; the pristine-radio fixture made discriminating.
- Review-methodology lesson of record (transferable): when a guard grows a conjunct, the mutation that matters removes the CONJUNCT, not the guard — whole-guard reverts fail the PRIOR round's test and mask the new half. The R1 pin ("SURVIVES A LEFTOVER LAYOUT") exists because the narrowed revert passed 104/104 before it.

### Verification

- Tier 1/gates: `npm test` 321/321 (was 315 at `4cf0a1c`), `tests/edit-grid.test.mjs` 105, 28 screenshot baselines at 0.000%. Every remedy pinned under an individually-reverted (narrowed) mutation failing exactly its own test.
- DISCLOSED SURVIVOR, ruled equivalent by the reviewer against a known-good tree (the equivalence rule from the "313/313 with the defect live" post-mortem requires the reason recorded, so: ) `ownsLayout = planned !== null` admits the mutant `= true` because the only path reaching the assignment with a null plan is the same `core.applyWidths` call whose `!active` branch just cleared `tableLayout` (`core.js:886-888`), so the guard's other conjunct is false regardless of the flag; and a REFUSED restore returns before the assignment is reached. Two other mutants of the line die (hard-wired declaration `true` fails the R1 pin; the `||` variant fails the axis slice, though for a sandbox ReferenceError, disclosed as such). Kept for state honesty and for being right under change.
- Tier 2 (fixture, real stylesheets, no extension): stamped control bar computed `inline-flex` 185×24 while an UNSTAMPED bare `div` of the same class computes `none` 0×0 — the hostile `netsuite.css` rule is live in the cascade and the SIGHTING FOUR override wins only through the ownership stamp; `col-hidden` on a header cell and an aligned body cell computes `none` on both and restores to `table-cell` clean. The design §8 "computed display, never a class name" obligation for hide/show is discharged at this tier.
- Tier 3 (live, `id=16342809&e=T`, extension at `bc8e370`, no save — the owner's mid-session offer to save the record, made to suppress the beforeunload prompt, was DECLINED: nothing was unsaved, the prompt is NetSuite's blanket edit-page guard): safety triple verified view-side then edit-side; control bar computed `inline-flex` 422×24; the PRIOR session's 4-column hidden set re-applied on mount (cross-reload persistence); Quantity hidden (the plan labels it required; the required-marker probe later showed this form stars only Item and Tax Code via `span.listheaderreq`) — header and body computed `none`, chip added, menu retained through an inside click and closed on an outside click; opening line 1 force-revealed every hidden column to computed `table-cell`; OK commit with columns hidden succeeded with values preserved (qty "1" readable inside computed-`none` cells, rows 1–3) and the hidden set re-applied across the commit repaint, the auto-focused PRISTINE entry row correctly NOT revealing; Remove renumbered 9→8 and the hidden set re-applied; navigate-away teardown restored all 9 data rows on the view URL (record byte-identical); re-entry seeded the menu from the stored set and the show path restored the owner's exact pre-session personalization. Zero SuiteMate console errors (NetSuite-native and packship-bundle noise present, no SuiteMate frames).
- Tier 4 (View Mode, same record): so-columns mounted (57 nodes), edit-grid 0 nodes, internal-ids present; the H1 headers-only class absent (9 of 10 rows match the 48-cell header count); `git diff main -- src/so-columns/ src/form-views/ src/csv-export/ src/tab-title/` is exactly the four one-token `FOREIGN_NODE_SELECTOR` additions of spec §4.4.
- Disclosed live deviations: reveal TOAST unobserved (read seconds after the click; reveal itself proven); add-line-by-typing skipped (dirty-entry quadrants unit-pinned in both directions for checkbox and radio; typing into the item picker fires sourcing on a production record; commit-repaint and remove-renumber cover new-node re-apply); the plan Step-3 literals `_fs`-spans-present / cells-in-`main_form` predate the M1.5 identity amendment and are false ON THIS FORM by measurement (static rows carry no `_fs` spans; the machine sits across the mini-form boundary); the `suiteMateV3ColumnOrder` byte-baseline was not captured pre-session (extension console unreachable from the bridge) — mode isolation rests on the route complement, the source-purity test, the reviewer's route-impossibility finding, and the view feature functioning identically after the session.

### Carried follow-ups (none blocking M3)

- `resolveScopeKey` fallback should be OBSERVABLE and fail closed on writes (spec §7 doctrine): the two key shapes differ by colon-segment count; first-install-under-fallback currently pins the hostname key for the mount (pre-existing exposure made sticky by the latch — reviewer-assessed net improvement).
- Harness containment should be DERIVED via the existing parent-walking `closest` rather than declared by `fire()`'s `outside` flag — opt-in fidelity is the shape that hid `repaintHeader`'s class omission.
- Menu lifecycle edges, unconfirmed reachability: `ensureControls` orphans a standing menu if the container is replaced without a teardown; a stale menu tick after a reseed no-ops silently.
- M2 checkpoint debt stands: recalc survival and horizontal-scroll residual live checks never ran; M2's entry was never written. This entry is not it.
- Schema note: the branch carries settings `SCHEMA_VERSION` 6 (Q1, owner-approved). v3.21.1 (`main`) refuses v6 settings by design — reverting the EXTENSION to main after running this build requires the recorded schema-downgrade console step. Live storage remains a one-way door (`save/CHECKPOINTS.md:1199`).


### Independent adjudication (Opus 5 verifier, 2026-08-04)

Verdict of record: **the M3 mechanism passes live; the M3 checkpoint is not yet declared.** Per-gate: mount/coexistence PASS; hide-at-computed PASS; show path PASS; persistence round trip PASS; force-reveal on open line PASS (toast INSUFFICIENT — unobserved, `revealToasted` is a one-shot latch and the read came 2–4s late); commit-with-hidden PASS at the design level with value preservation INSUFFICIENT (the committed line was pristine — a no-op commit cannot be ruled out); re-apply after commit repaint PASS; re-apply after remove-renumber PASS; menu dismissal PASS (synthetic dispatch proves wiring, not hit-testing; Escape untested); Tier-4 View regression INSUFFICIENT (mount-and-quiet was shown, behaviours were not); fixture computed sweep PASS (write-count half rests on prior Task 16 evidence); safety/no-save PASS with the discard PROVEN. Adjudicator independently confirmed the pristine-entry-row non-reveal and the `_fs`/mini-form deviations as design-true, and confirmed `git diff --name-only main | grep so-columns` returns `tests/so-columns.test.mjs` in addition to the sanctioned one-token `core.js` change — the plan's "exactly one path" literal is stale; the brief's no-View-module-modified constraint HOLDS.

Coverage owed before the checkpoint is declared (adjudicator's R-list): **R1** edit a visible field that drives a recalc, commit, confirm value + recalc + re-apply (closes 6b and the brief's edit/recalc behaviours); **R2** add a line with a column hidden (or record the gap as un-run — "add/remove re-applied" is not supportable as drafted); **R3** capture the force-reveal toast on the FIRST reveal after a fresh load, read within ~1s; **R4** behavioural Tier-4 — sort, filter, a width change, personalization, Export view producing data rows, tab titles; **R5** `suiteMateV3ColumnOrder` byte-baseline captured, or the byte-identical claim struck from M3 and bound as a hard M5 gate (spec §9 places the H2 check "from M5 onward"; the plan's Task 17 Step 4 demand at M3 is the stale half).

Claim corrections applied on the adjudicator's ruling: Quantity is NOT a required column on this form (the required marker is `span.listheaderreq`; only Item and Tax Code carry it) — the plan's "hide a required column" language tracked the spec's generic form, and this entry's earlier "(required)" label was corrected; the probe-11 "validation failure force-reveals" mechanism was REPLACED BY PREVENTION in the as-built design and is not claimed; "Tab-into-hidden reveals one column" is unimplementable on this form (widgets materialise per cell) and is struck, not deferred; Task 18's `ℹ tests 226` expectation is stale arithmetic (the suite is 321+).

### Owner directive, 2026-08-04 (post-adjudication, in scope for M3 before declaration)

While testing live, the owner directed: **columns NetSuite stars as mandatory must not be hideable.** Ground truth probed on the locked form: the star is `span.listheaderreq` inside the header cell; Item and Tax Code carry it; the set is form-specific and must be DOM-derived per install. Implementation round dispatched (menu row checked+disabled as an A3.2 carve-out, apply exemption with the stored value RETAINED per §7, no chip, choke-point write refusal, per-column not global). The owner's other live observation — hidden columns reappearing while typing a new line — was initially retained as the designed dirty-entry prevention, and was then REVERSED by a second owner directive the same day (see the M3 declaration below): the force-reveal is deleted at `c09997a` and hidden now stays hidden through every Edit Mode interaction.

### M3 DECLARED — close-out session, 2026-08-04 (extension at `c09997a`)

The adjudicator's R-list is closed and both same-day owner directives are live-verified on the locked order; every claim below is at computed style or real UI output, no save, record byte-identical (navigate-away discard re-proven: the view page shows the same 9 data rows after the session's in-page add, edits and remove).

- **Owner directive 1 (`08e3c1e`) live:** the Columns menu renders exactly Item and Tax Code checked + disabled + titled "Required column — cannot be hidden" (the two `span.listheaderreq` carriers on this form); all 41 other columns enabled. Escape closes the menu (the adjudicator's untested-path note, now exercised).
- **Owner directive 2 (`c09997a`) live — hidden stays hidden:** with three columns hidden, the set held at computed `none` through: line open; field typing; a qty 1→2 commit; the commit repaint; a second commit reverting 2→1; entry-row typing of "MCH14" (the owner's reported broken scenario, verbatim); two NetSuite validation alerts (both fully visible with columns hidden — the accepted-trade path observed live, twice); an item sourcing rewrite; an add that committed (row count 9→10, the hide re-applied to the brand-new row's cells); and a remove (10→9). Chips and control bar stable throughout.
- **R1 (edit + recalc) CLOSED:** the qty commit recalculated Amount 32.00 / Tax 2.91 and fired NetSuite's reorder-point advisory — server-side recalc, value readable in the machine, hidden set re-applied after. Converts the prior adjudication's 6b INSUFFICIENT.
- **R2 (add a line) CLOSED:** committed add with a column hidden; row-count-increase repaint re-applies the hide to the new row. The added line was then removed in-page and the whole session discarded.
- **R3 (toast) CONVERTED BY DESIGN:** the reveal toast was deleted with the force-reveal; the replacement claim — no reveal at all — is unit-pinned and live-verified above.
- **R4 (Tier-4 behavioural) CLOSED:** sort Z→A produced a real reorder and Clear sort restored the exact native order; unchecking one filter value hid exactly that row at computed level and Clear filter restored all 9; Personalize entered (Done/Reset/hint) and exited; a width drag moved a header 77→107px and back; **Export view exported 9 visible rows** to `salesord-16342809-view.csv` (the headers-only class is dead at the live level); the tab title is formatted. Zero SuiteMate console errors in either mode (four `SSS_SEARCH_ERROR_OCCURRED` exceptions from the packship bundle, no SuiteMate frames).
- **R5 (mode isolation) BOUND TO M5:** the `suiteMateV3ColumnOrder` byte-baseline was not captured (the owner skipped the service-worker console step; the key is unreachable from the page bridge). Per the adjudicator's own alternative and spec §9 (H2 check "from M5 onward"), the byte-identical claim is struck from M3 and stands as a hard M5 entry gate. The static guarantees remain: the literal cannot appear in `src/edit-grid/` (source-purity pins at both files) and the route complement is mutually exclusive by construction.
...
## Edit Mode hide/show: segment-paged machines (post-M3, owner-reported)

Status: Complete; reviewed to MERGE; live-verified on the locked record and the owner-designated 202-line order

Date: 2026-08-04 · Commits `72a67ca` (fix) on `feature/edit-mode-table-enhancements`

### Included

- Owner edge case: on Sales Orders with many item rows, hidden columns reappeared when adding lines. Live-reproduced pre-fix on the owner-designated 16365465 (form B, 202 lines, `inpt_item_segment_select` machine): selecting segment "26 - 50 of 202" replaced the machine's header and tbody and every hidden column rendered visible (headerHiddenCount 0, measured). Root cause: the identity corroboration sampled only the FIRST EIGHT data lines while samples are indexed by each row's own line number — on any segment past the first, zero rendered rows fell inside the sampled lines, the no-sample pre-gate refused, the install declined, and the swapped-in DOM never got the re-apply. Adding a line lands on the LAST segment, hence the owner's report.
- The fix moves the eight-row sampling cap from "the first 8 lines of the data" to "the first 8 RENDERED rows" (parse keeps every line under a private 1000-line pathology cap that degrades rather than refuses; the sparse line-indexed samples shape is untouched, keeping sampleTexts[row] and column.values[row] the same line on any segment). sameColumnIds (single consumer) became axisCompatible: a sparser segment's derivation may HOLE positions the pin resolved without latching; holes defer to the pin, the pin is never upgraded mid-mount, real conflicts and width changes latch exactly as before, and compatibility returns THE PIN. Disclosed residual (reviewer-accepted): a genuine layout change at a position the fresh derivation happens to hole survives until teardown — narrow, and the alternative is the feature switching itself off because the user paged.
- Un-capping the parse exposed a real perf cliff the implementer measured and closed unbriefed: the sparse index-space sweep touched every hole once per (label × candidate) pair — 10.1M string reads, 1182ms per repaint-driven install at the cap. A hoisted present-row census (doubling as the no-sample pre-gate) makes correlation ~40ms regardless of which segment renders, pinned as a COUNT INEQUALITY (holeReads ≤ slots) after review measured a strictly-cheaper hole-skipping census failing an exact-count pin with a message claiming the opposite of what happened.
- Process incident, disclosed: the captain wiped the round's uncommitted core.js with a `git checkout <file>` during a mutation probe; the implementer's own pre-revert scratch snapshots restored it exactly, verified item-by-item against the spec and the 116-test oracle. Standing rule adopted: no `git checkout <path>` in a tree carrying uncommitted rounds — scratch copies before every in-place revert, both directions.

### Verification

- Review MERGE with two Minors, both landed and mutation-verified: the T5 ceiling relaxed to an inequality (still kills the restored index sweep: 95,677 reads against a 400 ceiling), and axisCompatible's length clause pinned by the one direction that can reach it — a WIDER derivation, invisible to the positional walk that runs over the pin's indexes (a narrower derivation cannot pin the clause: undefined at the pin's last index is already refused by the null checks). Revert table re-run post-rebuild, identical: S-1 restore-the-slice → 4 fail including the fixture DECLINE and the lost-hide-on-swap; S-2 collection cap → T1 alone; S-3 strict pin equality → T3 alone; S-4 index sweep → T5 alone.
- Live, post-reload, extension at `72a67ca`: locked record regression PASS (43-axis, zero holes, 28 stored hides + chips applied). Owner-designated 16365465: segment 1 → 26 hidden; segment 2 — the exact pre-fix reproduction — 26 HELD with body cells computed none; last segment (201-202) held with the ENTRY ROW's hidden cells none; typed MCH376 into the entry row (sourcing rewrite: held); committed → segment "201 - 203 of 203", item_row_203 created WITH the hide applied to every hidden column of the brand-new row; removed the line → 202 restored, hides held through NetSuite's automatic segment jump. The accepted-trade residual appeared live (an Amount validation alert with the widget in a closed floating editor) and was resolved through the visible header — on this form Amount is not hidden; the two raw-id chips (custcol_anx_*, allocationalert) are §7 retention rendering ids that are not on this form's axis. Navigate-away discard proven: the view page renders all 202 rows. Zero SuiteMate console errors across the whole session.
- 332/332 tests, edit-grid file 116, verify green, 28 baselines 0.000%.
...
## Styling series checkpoint — parity UI, theme default, totals, active tabs (2026-08-04)

Status: Complete; every round adversarially reviewed to MERGE; owner visual passes live for the parity UI, remaining surfaces at the next reload

Commits `6e552ab` (Edit Mode UI parity: Reset, per-header ▼ menus with View's owner-tracked one-click semantics, View's styling values under edit-grid's own classes), `17de71c` (default primary #607799 → #5058f4: DEFAULTS + every var() fallback + popup seeds + V1 hashes + 24 baselines regenerated on a pre-verified 0.000% tree), `9b0aec1` (transaction totals as a SuiteMate component + one active-tab treatment across top nav and subtabs).

Ledger notes carried from review, for whoever meets them next: the parity round's naturalWidths measurement now includes the header arrow (constant, cannot walk — comment states the argument); the totals Total row is POSITIONAL (tbody tr:last-child) and on a form whose last row is not a Total the emphasis lands on the wrong figure — cosmetic, owner-directed, named here so an estimate form does not surprise; a separator as the last row would take the Total band (latent specificity ordering, one line if ever seen live); .bgtabbar/div.bgsubtabbar and real Redwood are live-only surfaces (the redwood fixture renders the CLASSIC path — no .isRedwood on its root; that baseline is not Redwood evidence); the active-tab root cause was the 17de71c palette swap orphaning --theme-main-light, which had also been an unmeasured a11y regression (2.6:1 active-tab text, now 8.93:1 AAA); helpers-are-fixtures lesson from the parity round (a test helper more capable than the production path masks the gap it stands in for).


## Zebra + unconditional tabs + guard tier (2026-08-05, `c90c40b`)

Status: Complete; two review passes to MERGE with every narrowed revert re-executed by the reviewer; the two WIP preservation commits squashed away

The round's own story is in the commit message. Ledger extracts for whoever meets them: the active-tab defect's true source was v3-compat.css (loads last, unconditional, --theme-main-light !important) — the earlier fix had won in fixtures only by an accident of guard specificity; three deliberate copies now exist with an equality pin comparing them (last-match extraction — the cascade takes the later duplicate, and a first-match reader passed a genuinely rendering drift). Zebra had shipped invisible for months (1.044:1) with parity counting non-data rows; the Tier-2 probe now enforces a perceptibility floor and forces all four carve-outs per fixture, and 16-of-17 striped fixtures had rendered too few rows for the parity check to mean anything until the record fixture gained an interleaved totals row. Carried: the pastel-theme floor limitation (comment at the tint token, upgrade path named); STRIPE_CHECK_FLOOR sits at the exact current count by design — the next machine-bearing fixture trips it intentionally; the reviewer did not eyeball the record-page render (0.000% against the new baseline; the owner's visual pass covers it); two more over-broad-git-checkout incidents this round (one captain, one implementer), both caught by the standing scratch-copy rule — the rule stays.


## Summary refinement live pass + zebra isRedwood fix (2026-08-05, `0479cb0`)

Status: Complete; owner-facing round closed with all three deliverables live-verified

The Summary box refinement (`65bb6d7`, 14 cascade-read value pins) verified live on the locked record: every spec value computed as pinned, and the owed theme-propagation proof ran — `--theme-main` moved to red inline on the live page and the caption fill and Total border-top followed TOGETHER, restoring cleanly, which is the "move the variable and both follow" evidence the round required. Zebra was then found invisible on every live page: real classic pages carry `.isRedwood` (the same over-firing detection behind the active-tab defect), and the `.isRedwood` token branch at netsuite.css:86 set `--row-odd-bg-color` = `--row-even-bg-color` = `--table-header-bg-color`, so the stripe rule fired and painted the base colour — while every fixture, which never carries the class, kept passing. Diagnosis was bottom-up: selector matched live (querySelectorAll 4, injected copy applied — outline rendered), so the fault had to be in token resolution, and it was. Fix per the owner's "never worry about Redwood" directive, same shape as the tabs: row tokens moved to the unguarded `html:not(.ext-f)` block (`0479cb0`). Narrowed revert added where the hole actually was: the Tier-2 stripe probe now re-resolves both shades with `.isRedwood` forced onto the fixture's root and fails under the 8/255 floor if a neutralising branch returns — mutation-tested by re-adding the branch (delta collapsed to 0/255, capture failed with the named error). Live post-reload: view and edit both alternate white / ~rgb(241,242,254) — the 8% tint of #5058f4, delta 14/255, matching the fixture-measured delta exactly. Ledger extracts: the redwood fixture DOES now exercise stripes but still renders the classic path (no `.isRedwood` on its root — the forced-class probe is the isRedwood evidence, not that baseline); one more over-broad `git restore` incident (the captain, mid-mutation-test, wiping the uncommitted fix) — recovery was exact because the intended bytes were already hash-pinned in verify.mjs before the mutation ran; the scratch-copy rule stays and now has a corollary: pin the hash BEFORE running a destructive mutation test, it is the cheapest possible restore point.
...
## View Mode Hide Columns performance (2026-08-05, `37dcba2`)

Owner-reported: hiding a column on a long item list took seconds while sort/filter/drag stayed fast. Root cause proven by isolation, not inference: applyWidths interleaved a layout read and a width write per header cell, and immediately after applyHidden changes the fixed-layout table's geometry every write invalidates and every next read forces a full reflow of the whole machine — 11.5s interleaved vs 184ms batched for the same work on the live 203x70 order (~69 reflows vs one). LoAF ruled out style/layout-in-frame and page scripts first; the page-world replica then reproduced the number, which is what separated this from the earlier candidates (row-loop toggles: 13ms; single reflow: 270ms — neither explained 4-12s). Fix is two-pass read-then-write in applyWidths, which is also the function's stated semantics ("freeze at current rendered width" — one snapshot). The batching is pinned by a unit test that records read/write order through stub cells and names the thrash in its failure message. Live after reload: hide 369ms/342ms flat across consecutive hides, show 144-194ms, sort 198ms, filter instant, small record 32/12ms, Edit Mode untouched (separate code, smoke-verified). Ledger note: the earlier so-columns re-measure-drift DEFECT pin (unstored columns growing by the border delta per apply) is unchanged by this fix — batching alters when reads happen, not what is written; the drift's fix remains future work behind its own pin.

## Active-nav tint — owner-corrected treatment (2026-08-05, `2a4ca74`)

The light/white active-tab surface shipped in the same branch was REJECTED by the owner ("completely white background with black text — I do not want this treatment") and replaced with the owner's exact mix: active = color-mix(in srgb, var(--theme-main) 68%, #fff), white text and arrows kept, identical on the uif top nav, the form tab row and every subtab (subtab-active token carries the same mix). No hardcoded hue; the tint follows theme changes. Literal pins updated to the tint, equality pins unchanged, 28 baselines regenerated; fixture render matches the owner's preferred example (lighter version of the surrounding nav colour). Live verification on both standing records happens at the next extension reload, folded into the Clean Commerce table round that follows. Ledger note: this is the third treatment for the same surfaces in one day (sink -> light -> tint); the pins made each swap total — every copy moved together each time, which is exactly what the equality block was built for.

## Clean Commerce item tables (2026-08-05, `58c4584`) — live-validated both records

The owner-approved "02. Clean Commerce" design, delivered on the existing systems rather than beside them: container chrome on the machine's own scroll box; header as machine-scoped overrides of the shared header tokens (sln follows free); zebra/hover as ROW-SCOPED token overrides at 4.5%/6% so lists keep their 8%; alignment taken from NetSuite's own align=right markup with zero JS. Live pass on both standing records with the theme at a NEVER-SHIPPED colour (#3956c9, the user's own change): every derived value — header rule, zebra mix, hover token, link colour, nav/subtab 68% tint — computed to that theme exactly, which retires the theme-propagation question for this whole surface family. Ledger extracts: Edit Mode keeps NetSuite's native ~25px row density (an edit-table height rule out-specifies the 46px minimum — deliberate, disclosed, and the safer reading of "preserve editing"); machines on non-transaction pages (saved-search builders, script sublists) carry the design too, consistent with every prior machine-wide treatment — "transaction sublists" read as "machine sublists" exactly as the zebra round read it; the stripe probe now resolves shades per group at the group's own rows (row-scoped tokens made body-resolution wrong) and its floor sits at 6/255 with the pastel ledger applying ~1.8x sooner at 4.5%; STRIPE_CHECK_FLOOR unchanged at 17 and all 17 pass under the split-tint world. The View Hide perf fix held at 30ms under the new cell rules.

## v3.24.0 release (2026-08-05)

Released from `feature/default-primary-6269e7`: default primary #6269e7 for fresh installs; the uif top nav's current section joins the active-tab family; owner-corrected active-state tint (68% theme / 32% white, white text, all scales — after a rejected white treatment); View Mode Hide Columns performance fix (batched applyWidths reads: 369ms vs 4.2-11.8s on the 203x70 order); Clean Commerce item tables (container/header/rows/typography, theme-derived zebra 4.5% + hover 6%, live-validated on both standing records under a never-shipped theme colour).
