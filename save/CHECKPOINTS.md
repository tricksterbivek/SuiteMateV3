...
Spec: `docs/superpowers/specs/2026-08-02-edit-mode-table-enhancements-design.md` **Amendment 1** (`:234-441`, sections A1.1-A1.7) · Plan: `docs/superpowers/plans/2026-08-02-m15-identity-amendment.md` · Built via Opus 5 subagent-driven development (implementer / reviewer per task); one further controller adjudication (**#13**, document-scoped input resolution) and escalation **#6** (the inert length guard) recorded in the SDD ledger `.superpowers/sdd/2026-08-02-m15-identity-amendment/progress.md`. Commits `21ae0bd..0f88827` on `feature/edit-mode-table-enhancements` — **every commit range in this entry is inclusive of both endpoints**, meaning the first commit *of that item* and its last, not git's exclusive `A..B` revision syntax. The ledger renders the same span git-exclusively as `bbb4479..0f88827`; both describe the same twelve commits, and `bbb4479` is the last plan-review commit, not the first task commit.
...
6. **Upgrade slice tests to behavioral tests** where M2 gives them real callers — `fieldIsDirty`, `forcedRows`, `queueApply`, `applyAll`, `enqueueSave`; four of those six caller-less seams have zero coverage today.
...
8. Smaller carried items due at M2: the `readColumnIdsFrom` selector-injection guard (`runtime.js:135`) is untested while `MACHINE_TABLE_SELECTOR` stays literal — one assertion, or `CSS.escape` as internal-ids does; the fixture round-trip harness still has no committed home and lives in two report appendices that have already diverged once; and M2 re-adds the two elided harness checks (console diagnostics, duplicate ids).
