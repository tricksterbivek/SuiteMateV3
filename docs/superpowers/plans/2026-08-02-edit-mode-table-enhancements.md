...
- Produces: `rowFilters: { [columnId]: { anyOf?: string[], op?, value? } }` (**session-only, never persisted**), `filterQueryText: { [columnId]: string }`, `applyCurrentFilters(table, columnIds)`, `forcedFilterRows(table, columnIds)`, `dataRowsOf(table, columnIds)`, `filtersActive()`, `openFilterMenu(table, columnId)`, `currentMenuColumnId()`, `setColumnFilter(columnId, next)`, `recomputeColumnFilter(columnId)`, `renderScopeNote(table, columnIds)`. M7 reuses `openFilterMenu` and `currentMenuColumnId` for its sort entries.
